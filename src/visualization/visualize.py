...
print(df[['median_house_value', 'price_per_person', 'area_sqft', 'high_end', 'price_category']].head())

# Étape 3 — GroupBy & Aggregation
print("\nÉtape 3 — GroupBy & Aggregation")

# Moyenne median_income et price_per_person par ocean_proximity et high_end
grouped_stats = df.groupby(['ocean_proximity', 'high_end']).agg({
    'median_income': 'mean',
...
df_prix = df[df['high_end'] == 'Oui'][['ocean_proximity', 'median_house_value', 'price_per_person', 'price_category']].copy()
df_info = df[df['ocean_proximity'] == 'NEAR BAY'][['ocean_proximity', 'median_income', 'housing_median_age', 'AveRooms', 'AveBedrms']].copy()
