...
def test_prepare_data(sample_data):
    ...
    X_train, X_test, y_train, y_test, transformer = preprocessor.prepare_data(sample_data)
...
def test_create_preprocessor(sample_data):
    ...
    transformer = preprocessor._create_preprocessor(numeric_features, categorical_features)
    
    assert isinstance(transformer, ColumnTransformer)
    assert len(transformer.transformers) == 2  # Utilisation de transformers au lieu de transformers_
