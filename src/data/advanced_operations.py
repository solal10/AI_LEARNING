...
np.random.seed(42)
ocean_proximity = np.random.choice(['NEAR BAY', 'INLAND', 'NEAR OCEAN', 'ISLAND', '<1H OCEAN'], size=len(df))
...
advanced_grouped_stats = df.groupby('ocean_proximity').agg({
...
df['haut_de_gamme'] = df['median_house_value'].apply(lambda x: 'Oui' if x > 3.5 else 'Non')
...
df_info = df[['id', 'ocean_proximity', 'housing_median_age', 'AveRooms', 'AveBedrms', 'population']].copy()
...
advanced_groupby = df.groupby(['ocean_proximity', 'haut_de_gamme']).agg({
...
time_analysis = df.groupby(['month', 'haut_de_gamme'])['median_house_value'].mean().unstack().round(2)
print(time_analysis)
